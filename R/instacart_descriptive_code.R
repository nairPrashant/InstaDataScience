---
title: "Instacart Descriptive Analysis"
...
orders %>% 
  ...
  geom_histogram(stat="count",fill="sienna2")
```
...
orders %>% 
  ...
  geom_histogram(stat="count",fill="olivedrab4")
```
...
```{r}
tmp2 <- orders %>% 
  filter(order_hour_of_day > 22 | order_hour_of_day<6) %>% 
  inner_join(order_products,by="order_id") %>%
  group_by(product_id) %>% 
  summarize(count2 = n()) %>% 
  left_join(select(products,product_id,product_name,department_id),by="product_id") %>%
  #filter(department_id != 4 & department_id != 16) %>% 
  top_n(10, wt = count2) %>%  
  arrange(desc(count2))

kable(tmp2)
pn2<-tmp2$product_name

tmp2 %>% 
  ggplot(aes(x=reorder(product_name,-count2), y=count2))+
...
tmp %>% 
  ggplot(aes(x=reordered,y=proportion,fill=reordered))+
...
```{r fig.width=4}
#If the product_name contains 'organic', product is organic
products <- products %>% 
...
products %>% 
  group_by(organic) %>%
  summarize(count = n()) %>%
  ggplot(aes(x=organic,y=count/sum(count), fill=organic))+
  geom_bar(stat="identity")
...
tmp %>% 
  ggplot(aes(x=organic,fill=organic,
             y=mean_reordered))+geom_bar(stat="identity")
```
...
#This gives all of the products grouped by department and aisles
tmp <- products %>% 
...
#This gives the products which have been ordered grouped by department and aisles 
tmp2<-order_products %>% 
  group_by(product_id) %>% 
  summarize(count=n()) %>% 
  left_join(products,by="product_id") %>% 
...
treemap(tmp2,
        ...
        vColor="department",
        palette="Set2",
        title="How are aisles organized within departments?",
        sortID="-sumcount", 
        border.col="#FFFFFF",
        type="categorical", 
        fontsize.legend = 0,
...
treemap(tmp,
        index=c("department","aisle"),
        vSize="n",
        title="How many unique products are offered in each department/aisle?",
        palette="Set3",
        border.col="#FFFFFF")
```
...
treemap(tmp2,
        index=c("department","aisle"),
        vSize="sumcount",
        title="Popular department/aisles",
        palette="Set3",
        border.col="#FFFFFF")
```
